...
export default function LoginPage() {
  ...
  // If token is already valid → redirect to dashboard instantly
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      particles.forEach(p => {
        ...
        ctx.fillStyle = `rgba(0,255,157,${p.alpha})`; ctx.fill()
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push("/dashboard")     // navigate to app
    } catch (err: any) {
  ...
  // While checking token validity (or redirecting) show nothing
  if (!ready) return (
    <div style={{ minHeight: "100vh", background: "#08080e", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        {[0, 1, 2].map(i => (
          <span key={i} style={{ width: 7, height: 7, borderRadius: "50%", background: "var(--accent)", animation: `glowPulse 1.2s ease-in-out ${i * 0.18}s infinite` }} />
        ))}
      </div>
    </div>
  )
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .stat-card{background:var(--bg3);border:1px solid var(--border);border-radius:var(--r);padding:14px 18px;display:flex;flex-direction:column;gap:3px;transition:border-color .2s,box-shadow .2s}
        .stat-card:hover{border-color:rgba(0,255,157,.3);box-shadow:0 0 16px rgba(0,255,157,.07)}

        .auth-input{background:var(--bg3);border:1px solid var(--border2);border-radius:var(--r);color:var(--text);padding:13px 40px 13px 16px;font-size:14px;font-family:var(--mono);outline:none;width:100%;transition:border-color .2s,box-shadow .2s}
        .auth-input:focus,.auth-input.focused{border-color:rgba(0,255,157,.5);box-shadow:0 0 0 3px rgba(0,255,157,.08)}

        .submit-btn{background:var(--accent);border:none;color:#000;padding:14px;border-radius:var(--r);font-size:15px;font-weight:700;font-family:var(--sans);width:100%;transition:box-shadow .2s,transform .2s,opacity .15s;display:flex;align-items:center;justify-content:center;gap:8px}
        ...
        @media(max-width:800px){
          ...
          .left-panel{flex:none!important;width:100%!important;padding:40px 24px!important;min-height:auto!important}
          .hide-mobile{display:none!important}
          .right-panel{width:100%!important;padding:40px 24px!important}
          .stat-row{gap:8px!important}
        }
        @media(max-width:400px){
          .left-panel,.right-panel{padding:32px 18px!important}
        }
      `}</style>
      ...
      <div style={{ minHeight: "100vh", background: "var(--bg)", opacity: mounted ? 1 : 0, transition: "opacity .35s" }}>
        <div className="auth-layout" style={{ display: "flex", minHeight: "100vh" }}>
          ...
          {/* ══ LEFT PANEL ══ */}
          <div className="left-panel" style={{ flex: 1, background: "var(--bg2)", borderRight: "1px solid var(--border)", display: "flex", alignItems: "center", padding: "60px 56px", position: "relative", overflow: "hidden" }}>
            <canvas ref={canvasRef} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none", zIndex: 0 }} />
            <div style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 1, backgroundImage: `linear-gradient(rgba(0,255,157,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(0,255,157,0.03) 1px,transparent 1px)`, backgroundSize: "44px 44px" }} />
            <div style={{ position: "absolute", left: 0, right: 0, height: 60, pointerEvents: "none", zIndex: 2, background: "linear-gradient(to bottom,transparent,rgba(0,255,157,.02),transparent)", animation: "scanV 8s linear infinite" }} />
            <div style={{ position: "absolute", top: "20%", left: "30%", width: 280, height: 280, background: "radial-gradient(circle,rgba(0,255,157,.06),transparent 70%)", filter: "blur(30px)", pointerEvents: "none", zIndex: 1, animation: "glowPulse 5s ease-in-out infinite" }} />
            ...
            <div style={{ position: "relative", zIndex: 3, maxWidth: 420, display: "flex", flexDirection: "column", gap: 32 }}>
              <Link href="/" style={{ fontFamily: "var(--mono)", fontSize: 16, fontWeight: 600, display: "inline-block", textDecoration: "none", color: "inherit" }}>
                <span style={{ color: "var(--accent)", textShadow: "0 0 14px rgba(0,255,157,.6)" }}>&lt;</span>ReviewAI<span style={{ color: "var(--accent)", textShadow: "0 0 14px rgba(0,255,157,.6)" }}>/&gt;</span>
              </Link>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <p className="hide-mobile" style={{ fontSize: 15, color: "var(--muted)", lineHeight: 1.7 }}>
                  Paste code. Get inline AI comments on every suspicious line. Security, performance, style — all flagged instantly.
                </p>
              </div>

              <div className="stat-row" style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                {[{ val: "10+", label: "languages" }, { val: "< 5s", label: "avg review" }, { val: "100%", label: "private" }].map(s => (
              ...
              <div className="hide-mobile" style={{ background: "var(--bg3)", border: "1px solid var(--border)", borderRadius: "var(--r)", padding: "14px 16px", fontFamily: "var(--mono)", fontSize: 12 }}>
                <div style={{ color: "var(--muted)", marginBottom: 6 }}><span style={{ color: "var(--accent)" }}>$</span> reviewai analyze ./auth.js</div>
          ...
          {/* ══ RIGHT PANEL ══ */}
          <div className="right-panel" style={{ width: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: "60px 52px", position: "relative", overflow: "hidden" }}>
            <div style={{ position: "absolute", top: "40%", right: "-60px", width: 200, height: 200, background: "radial-gradient(circle,rgba(0,255,157,.04),transparent 70%)", pointerEvents: "none" }} />

            <div style={{ width: "100%", maxWidth: 360, position: "relative", zIndex: 1 }}>

              <div style={{ marginBottom: 32, animation: "fadeUp .6s cubic-bezier(.22,1,.36,1) .15s both" }}>
                <h1 style={{ fontFamily: "var(--sans)", fontSize: 30, fontWeight: 800, letterSpacing: "-1px", marginBottom: 8 }}>Welcome back</h1>
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                {/* Error */}
                {error && (
                  <div className="error-box" style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(248,113,113,0.07)", border: "1px solid rgba(248,113,113,0.25)", borderRadius: "var(--r)", padding: "11px 14px" }}>
                ...
                <div style={{ animation: "fadeUp .6s cubic-bezier(.22,1,.36,1) .34s both" }}>
                  <button type="submit" className="submit-btn" disabled={loading} style={{ opacity: loading ? 0.75 : 1 }}>
                    {loading ? (
                      <>
